...
// The next four are about particular characters rather than about the cast as
// a whole, so they are looked up rather than named. Naming them compiles them
// in, which stops this file building at all in a repository exported for a
// game cast that isn't the Mega Drive one; looking them up lets the check sit
// out a build that doesn't ship them.
func who(_ id: String) -> Personality? { Personality.all.first { $0.id == id } }

if let axel = who("axel"), let axel1 = who("axel1") {
    check("both Axels are distinguishable", axel.name != axel1.name,
          "\(axel.name) / \(axel1.name)")
}
if let sonic = who("sonic") {
    check("Sonic is the fastest thing here",
          Personality.all.allSatisfy { $0.id == "sonic" || $0.roaming.speed <= sonic.roaming.speed },
          "\(sonic.roaming.speed)")
}
...
if let maxsor = who("maxsor"), let skate = who("skate") {
    check("Max is slower than Skate", maxsor.roaming.speed < skate.roaming.speed)
}
if let earl = who("earltje"), let toejam = who("toejam") {
    check("Earl does not hurry",
          earl.roaming.speed < toejam.roaming.speed
              && earl.beatRange.lowerBound > toejam.beatRange.lowerBound)
}
